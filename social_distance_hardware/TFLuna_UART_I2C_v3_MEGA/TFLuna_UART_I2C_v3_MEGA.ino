...
void processDetection() {
  ...
  if ((distance1 >= MAX_DISTANCE_TO_DETECTED && distance1 <= MAX_DISTANCE_SENSOR)) {   
    if (info_detected_1.distance > 0 && info_detected_1.releaseTime == 0 && info_detected_2.distance > 0) { 
      info_detected_1.releaseTime = millis() - info_detected_1.time;       
    }
  ...
  if ((distance2 >= MAX_DISTANCE_TO_DETECTED && distance2 <= MAX_DISTANCE_SENSOR)) {    
    if (info_detected_2.distance > 0 && info_detected_2.releaseTime == 0 && info_detected_1.distance > 0) { 
      info_detected_2.releaseTime = millis() - info_detected_2.time;     
    }    
  ...
  if (info_detected_1.distance == 0 && info_detected_2.distance == 0) {
    long diffR = info_detected_2.releaseTime - info_detected_1.releaseTime;

//    if (diffR != 0)  {      
//      Serial.print("diffR: ");
//      Serial.print(abs(diffR));
//      Serial.print(" release 1: ");
//      Serial.print(info_detected_1.releaseTime);            
//      Serial.print(" release 2: ");
//      Serial.print(info_detected_2.releaseTime);   
//      Serial.println();   
//    }

    if (diffR < -70 && (info_detected_2.time > info_detected_1.time)) {      
      Serial.print("I");                           
      digitalWrite(LED, HIGH);
      in_sound();          
    } 
    
    if (diffR > 70 && (info_detected_1.time > info_detected_2.time)) {      
      Serial.print("D");                           
      digitalWrite(LED, HIGH);
      out_sound();            
    }
    ...
    info_detected_1.releaseTime = 0;
    info_detected_2.releaseTime = 0;  
  }
}
